...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Package trained model artifacts for deployment"
    )
    ...
    p.add_argument(
        ...
        required=True,
        type=Path,
        help="Directory containing trained PEFT adapter (trainer.save_model output)",
    )
    p.add_argument(
        "--output-dir",
        required=True,
        type=Path,
        help="Destination directory for the packaged artifacts",
    )
    p.add_argument(
        "--mode",
        choices=["adapter", "merged"],
        default="adapter",
        help="Export mode: adapter (default) or merged full weights",
    )
    p.add_argument(
        "--include-tokenizer",
        action="store_true",
        help="Also save tokenizer alongside package",
    )
    p.add_argument(
        "--adapter-name",
        default="default",
        help="Adapter name if non-default (for informational metadata)",
    )
    return p.parse_args()


def package_adapter_only(
    base_model: str,
    adapter_dir: Path,
    output_dir: Path,
...
def package_merged(
    base_model: str, adapter_dir: Path, output_dir: Path, include_tokenizer: bool
) -> None:
...
def main() -> None:
    ...
    if args.mode == "adapter":
        ...
        package_merged(
            args.base_model, args.adapter_dir, output_dir, args.include_tokenizer
        )
