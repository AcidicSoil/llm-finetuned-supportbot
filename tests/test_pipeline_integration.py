...
class _FakeTok:
    ...
    def __call__(
        self, batch, padding=True, truncation=True, max_length=8, return_tensors=None
    ):
...
def _rec(i: int, src: str, tag: str) -> DataRecord:
    return DataRecord(
        ...
        meta=Meta(
            source=src, timestamp=datetime(2024, 1, 1, tzinfo=timezone.utc), tags=[tag]
        ),
    )
...
def test_end_to_end_small_pipeline():
    # Build small dataset
    data = [
        _rec(i, "web" if i % 2 == 0 else "forum", "alpha" if i % 3 == 0 else "beta")
        for i in range(10)
    ]
    ...
    # Tokenize pairs
    out = tokenize_pairs(
        data, _FakeTok(), max_length=6, padding="max_length", truncation=True
    )
